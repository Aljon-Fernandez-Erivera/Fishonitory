    ...
    <section className="dashboard-page">
      <div className="page-heading">
        <div>
          ...
          <p>Manage attendance for each staff member.</p>
        </div>
        <button
          type="button"
          onClick={() => {
      ...
      <div className="calendar-card">
        ...
        <div className="calendar-grid">
          {calendarDays.map((calendarDay, index) => {
            if (!calendarDay)
              return <div className="calendar-empty" key={`empty-${index}`} />;
            ...
            return (
              ...
              >
                ...
                <div className="calendar-statuses">
                  {dayRecords.some((record) => record.status === "Present") && (
                    <i className="status-present">P</i>
                  )}
                  {dayRecords.some((record) => record.status === "Late") && (
                    <i className="status-late">L</i>
                  )}
                  {dayRecords.some((record) => record.status === "Absent") && (
                    <i className="status-absent">A</i>
                  )}
                  {dayRecords.some((record) => record.status === "Leave") && (
                    <i className="status-leave">L</i>
                  )}
                  {dayRecords.some((record) => record.status === "DayOff") && (
                    <i className="status-dayoff">D</i>
                  )}
                </div>
              </button>
            );
      ...
      >
        ...
        <form onSubmit={(event) => event.preventDefault()}>
          ...
          <button type="button" onClick={() => setDayFormOpen(false)}>
            Close
          </button>
        </form>
      </dialog>
