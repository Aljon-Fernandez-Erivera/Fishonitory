...
async function apiRequest(path, options = {}) {
  ...
  if (!contentType.includes("application/json")) {
    throw new Error(
      `API returned a non-JSON response (${response.status}). Restart the backend server.`,
    );
  }
...
function OwnerDashboard() {
  ...
  const [notes, setNotes] = useState(() =>
    JSON.parse(localStorage.getItem("ownerNotes") || "[]"),
  );
  ...
  const handleDeleteAttendance = async (id) => {
    if (
      !window.confirm("Delete this attendance record? This cannot be undone.")
    )
      return;
    try {
  ...
  const renderActivePage = () => {
    switch (activePage) {
      ...
      case "notes":
        return <Notes notes={notes} onAddNote={handleAddNote} />;
      case "sales":
        return (
          <Sales
            sales={sales}
    ...
    <main className="owner-dashboard">
      <aside className="owner-sidebar">
        ...
        <nav className="sidebar-nav" aria-label="Owner dashboard navigation">
          <button
            className={
              activePage === "overview" ? "nav-button active" : "nav-button"
            }
            type="button"
          ...
          <button
            className={
              activePage === "attendance" ? "nav-button active" : "nav-button"
            }
            type="button"
          ...
          <button
            className={
              activePage === "staff" ? "nav-button active" : "nav-button"
            }
            type="button"
          ...
          <button
            className={
              activePage === "inventory" ? "nav-button active" : "nav-button"
            }
            type="button"
          ...
          <button
            className={
              activePage === "tanks" ? "nav-button active" : "nav-button"
            }
            type="button"
          ...
          <button
            className={
              activePage === "sales" ? "nav-button active" : "nav-button"
            }
            type="button"
          ...
          <button
            className={
              activePage === "operations" ? "nav-button active" : "nav-button"
            }
            type="button"
