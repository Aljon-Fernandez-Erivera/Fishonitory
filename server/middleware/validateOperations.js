...
const validatePurchase = validate((req) => {
  ...
  string(req.body?.supplierContact, "Supplier contact", { required: false, max: 120 });
  string(req.body?.invoiceNumber, "Invoice number", { required: false, max: 80 });
  ...
  if (!Array.isArray(req.body?.items) || !req.body.items.length || req.body.items.length > 100)
    throw Object.assign(new Error("Purchase must contain between 1 and 100 items."), { statusCode: 400 });
  req.body.items.forEach((item) => {
    objectId(item.fishId, "Inventory item ID");
    number(item.quantity, "Purchase quantity", { integer: true, min: 1, max: 100000 });
...
const validateMortality = validate((req) => {
  ...
  number(req.body?.quantity, "Mortality quantity", { integer: true, min: 1, max: 100000 });
