  ...
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({
          message: "A staff member and valid payroll period are required.",
        });
    }
    ...
    if (!Number.isFinite(rate) || rate < 0 || rate > 100000) {
      return res
        ...
        .json({
          message:
            "Daily rate must be a valid non-negative number up to ₱100,000.",
        });
    }
    ...
    if (!Array.isArray(benefitItems) || benefitItems.length > 30) {
      return res
        .status(400)
        .json({
          message: "Deduction items must be a valid array (max 30 items).",
        });
    }
    ...
    for (const item of benefitItems) {
      ...
      if (!itemName || itemName.length > 80) {
        return res
          ...
          .json({
            message:
              "Each payroll adjustment must have a valid name (up to 80 characters).",
          });
      }
      if (
        ...
        return res
          .status(400)
          .json({
            message: `Invalid deduction amount for '${itemName}'. Must be greater than 0.`,
          });
      }
      if (!["addition", "deduction"].includes(item?.type)) {
        return res
          ...
          .json({
            message:
              "Each payroll adjustment must be an addition or deduction.",
          });
      }
    ...
    if (typeof notes !== "string" || notes.length > 1000) {
      return res
        .status(400)
        .json({ message: "Payroll notes must be 1,000 characters or fewer." });
    }
    ...
    const records = await Attendance.find({
      userId: staff._id,
      dateKey: { $gte: periodStart, $lte: periodEnd },
    });
    const lateDeductionTotal = records
      .filter((record) => record.status === "Late")
      .reduce((total, record) => total + (record.lateDeductionAmount || 0), 0);

    if (lateDeductionTotal > 0) {
      items.push({
        name: "Automatic Late Deductions",
    ...
    const baseGross = payableDays * rate;
    const grossPay = baseGross + benefits;
    ...
    if (netPay < 0) {
      return res
        ...
        .json({
          message:
            "Total deductions exceed gross pay. Net pay cannot be negative.",
        });
    }
    ...
    await writeAudit(
      ...
      "Payroll",
      payroll._id,
      `Payroll prepared for ${staff.staffName} (${period})`,
    );
    return res
      .status(201)
      .json({ message: "Payroll saved successfully.", payroll });
  } catch (error) {
    return next(error);
  }
};
